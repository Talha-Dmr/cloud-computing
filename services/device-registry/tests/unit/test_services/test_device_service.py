...
class TestDeviceService:
    ...
    @pytest.mark.asyncio
    async def test_create_device_duplicate_id(self, device_service, sample_device, sample_device_data):
    ...
    async def test_list_devices_with_pagination(self, device_service):
        ...
        assert device_ids == ["device-000", "device-001", "device-002", "device-003", "device-004"]
    ...
    async def test_list_devices_by_status(self, device_service):
        ...
        await device_service.create_device("active-device", "Active Device", DeviceType.SENSOR)
        await device_service.create_device("offline-device", "Offline Device", DeviceType.SENSOR)
    ...
    async def test_list_devices_by_type(self, device_service):
        ...
        await device_service.create_device("sensor-device", "Sensor Device", DeviceType.SENSOR)
        await device_service.create_device("actuator-device", "Actuator Device", DeviceType.ACTUATOR)

        sensor_devices = await device_service.list_devices(device_type=DeviceType.SENSOR)
        actuator_devices = await device_service.list_devices(device_type=DeviceType.ACTUATOR)
    ...
    async def test_update_device_status(self, device_service, sample_device):
        ...
        updated_device = await device_service.update_device_status(sample_device.device_id, new_status)
    ...
    async def test_authenticate_device_success(self, device_service, sample_device):
        ...
        with patch.object(device_service, '_validate_device_credentials') as mock_validate:
            mock_validate.return_value = True

            result = await device_service.authenticate_device(
                sample_device.device_id,
                "valid-secret-or-token"
            )

            assert result is True
            mock_validate.assert_called_once_with(sample_device.device_id, "valid-secret-or-token")

    @pytest.mark.asyncio
    async def test_authenticate_device_not_found(self, device_service):
        """Test authenticating non-existent device returns False"""
        result = await device_service.authenticate_device("non-existent-device", "any-secret")
        assert result is False

    @pytest.mark.asyncio
    async def test_authenticate_device_invalid_credentials(self, device_service, sample_device):
    ...
    async def test_get_device_statistics(self, device_service):
        ...
        await device_service.create_device("active-sensor", "Active Sensor", DeviceType.SENSOR)
        await device_service.create_device("offline-sensor", "Offline Sensor", DeviceType.SENSOR)
        await device_service.create_device("active-actuator", "Active Actuator", DeviceType.ACTUATOR)

        # Update statuses
        await device_service.update_device_status("active-sensor", DeviceStatus.ACTIVE)
        await device_service.update_device_status("active-actuator", DeviceStatus.ACTIVE)
    ...
    async def test_search_devices_by_name(self, device_service):
        ...
        await device_service.create_device("temp-sensor-1", "Temperature Sensor 1", DeviceType.SENSOR)
        await device_service.create_device("temp-sensor-2", "Temperature Sensor 2", DeviceType.SENSOR)
        await device_service.create_device("humidity-sensor", "Humidity Sensor", DeviceType.SENSOR)
    ...
    async def test_batch_update_status(self, device_service):
        ...
        for i in range(3):
            ...
            await device_service.create_device(device_id, f"Device {i}", DeviceType.SENSOR)

        # Update all devices to ACTIVE status
        updated_count = await device_service.batch_update_device_status(device_ids, DeviceStatus.ACTIVE)
